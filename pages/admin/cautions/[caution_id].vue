...
const id = computed(() => {
  const rawId = route.params.caution_id;
...
type CautionSchema = InferType<typeof schema>

//3. REACTIVE STATE

const cautionState = reactive({
  id: '',
  created_at: '',
  updated_at: '',
  description: ''
...
async function fetchCautionData() {
  try {
    if (!id.value) return;
    
    const data = await apiCall<Caution>(
      `${api_route}/cautions/${id.value}`, 
      'GET', 
      undefined, 
    ...
    if (data) {
      // Update caution data
      cautionState.description = data.description || '';
      cautionState.id = data.id || '';
      cautionState.created_at = data.created_at || '';
      cautionState.updated_at = data.updated_at || '';
...
async function handleUpdateCaution(event: FormSubmitEvent<CautionSchema>) {
  await apiCall<Caution>(
    `${api_route}/cautions`, 
    'PUT', 
    event.data, 
    { customSuccessMessage: 'Caution updated successfully' }
  );
}
...
async function handleAddProtocols(event: FormSubmitEvent<any>) {
  await apiCall<Caution>(
    ...
    { 
      ...
      onSuccess: () => fetchCautionData()
    }
  );
}

async function handleRemoveProtocol(protocol_id: string, caution_id: string, options?: ApiOptions) {
   return apiCall<Caution>(`${api_route}/protocols/${protocol_id}/cautions/${caution_id}`, 'DELETE',undefined, {
...
const isLoading = computed(() => cautionState.id != '');
...
const items = (row: Protocol) => [
  [{
    ...
    click: () => {handleRemoveProtocol(row.id, cautionState.id)}
  }]
]
...
<template>
  ...
  <UContainer class="flex flex-col gap-6 basis-auto min-w-max">
   ...
   <UContainer class="my-10 w-2xl ">
    <UForm :schema="schema" :state="cautionState" class="space-y-4" @submit="handleUpdateCaution">
    <UFormGroup label="Caution Description" name="description" class="mb-4">
      <UInput v-model="cautionState.description" />
    </UFormGroup>
